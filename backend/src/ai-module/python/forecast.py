...
def arima_forecast(product_id, sales_data, days=30):
    ...
    try:
        ...
        return {"status": "error", "message": f"Failed to create ARIMA forecast: {e}"}

# ---------------- LSTM training & prediction ----------------
def create_lstm_sequences(series, seq_len=14):
    """Create overlapping sequences from a 1D array/series for LSTM training."""
    seq = []
    for i in range(len(series) - seq_len):
        seq.append(series[i:i+seq_len+1])  # seq_len inputs + 1 target
    seq = np.array(seq)
    X = seq[:, :-1]
    y = seq[:, -1]
    return X, y

def train_lstm_model(product_id, sales_data, seq_len=14, epochs=50, batch_size=16):
    if not TF_AVAILABLE:
        return {"status": "error", "message": "TensorFlow is not available in this environment."}
    ...
    if len(series) < seq_len + 2:
        return {"status": "error", "message": "Not enough data to train LSTM."}
    
    # normalize using a scaler saved per product
    value_scaler = StandardScaler()
    series_scaled = value_scaler.fit_transform(series.reshape(-1, 1)).flatten()
    
    X, y = create_lstm_sequences(series_scaled, seq_len=seq_len)
    ...
    model_path = os.path.join(MODEL_DIR, f'lstm_model_{product_id}')
    scaler_path = os.path.join(MODEL_DIR, f'lstm_scaler_{product_id}.joblib')
    
    model.save(model_path)
    joblib.dump(value_scaler, scaler_path)
    
    return {"status": "success", "message": f"LSTM model for product {product_id} trained."}

def predict_lstm(product_id, sales_data, days=30, seq_len=14):
    if not TF_AVAILABLE:
        return {"status": "error", "message": "TensorFlow is not available in this environment."}
    ...
    if not os.path.exists(model_path) or not os.path.exists(scaler_path):
        return {"status": "error", "message": f"No LSTM model found for product {product_id}"}
    
    # load model and scaler
    model = load_model(model_path)
    value_scaler = joblib.load(scaler_path)
    ...
    series_scaled = value_scaler.transform(series.reshape(-1,1)).flatten()
    ...
    preds = value_scaler.inverse_transform(np.array(preds).reshape(-1,1)).flatten()
    ...
    for i, date in enumerate(dates):
        daily_data.append({
            "date": date.strftime("%Y-%m-%d"),
            "demand": int(round(preds[i]))
        })
    
    total_demand = int(round(np.sum(preds)))
    avg_daily = total_demand / len(preds)
    ...
    result = {
        ...
        "average_daily_demand": round(avg_daily, 2),
        "confidence_level": 95,
        "lower_bound": int(round(total_demand * 0.9)),
        "upper_bound": int(round(total_demand * 1.1)),
        "daily_forecast": daily_data,
        "model": "lstm"
    }
    return result

# ---------------- Ensemble updated to include LSTM ----------------
def ensemble_forecast(product_id, sales_data, days=30):
    ...
    lstm_result = predict_lstm(product_id, sales_data, days) if TF_AVAILABLE else { "status":"error","message":"TensorFlow not available" }
    
    # prefer available forecasts; average where possible
    candidates = []
    daily_lists = []
    
    for r in (rf_forecast, arima_result, lstm_result):
        if isinstance(r, dict) and 'daily_forecast' in r and isinstance(r['daily_forecast'], list):
            daily_lists.append(r['daily_forecast'])
    
    if not daily_lists:
        return { "status":"error","message":"No forecasts available" }
    
    # align length and average
    n = len(daily_lists[0])
    ensemble_daily = []
    for i in range(n):
        vals = []
        for dl in daily_lists:
            if i < len(dl):
                vals.append(dl[i]['demand'])
        avg_val = int(round(sum(vals)/len(vals)))
        ensemble_daily.append({
            "date": daily_lists[0][i]['date'],
            "demand": avg_val
        })
    
    total_demand = sum(d['demand'] for d in ensemble_daily)
    avg_daily = total_demand / len(ensemble_daily)
    
    result = {
        "product_id": product_id,
        "period": days,
        "start_date": ensemble_daily[0]['date'],
        "end_date": ensemble_daily[-1]['date'],
        "total_demand": total_demand,
        "average_daily_demand": round(avg_daily, 2),
        "confidence_level": 95,
...
def optimize_inventory(product_data, forecast_data):
    ...
    for _, product in products.iterrows():
        ...
        daily_demand = forecast['average_daily_demand'].values[0] if 'average_daily_demand' in forecast.columns else forecast['daily_forecast'].apply(lambda x: x['demand']).mean() if 'daily_forecast' in forecast.columns else 0
...
if __name__ == "__main__":
    print("This module provides RF, ARIMA, and optional LSTM forecasting utilities.")    
    model = RandomForestRegressor(n_estimators=100, random_state=42)
    model.fit(X_scaled, y)
